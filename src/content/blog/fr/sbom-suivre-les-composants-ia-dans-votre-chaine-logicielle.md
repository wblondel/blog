...
Si une organisation intègre un modèle IA tiers via une API, ou héberge un modèle open source en interne, l'absence d'un AI-BOM pourrait bientôt la disqualifier de contrats entreprise majeurs et l'exposer à de lourdes sanctions réglementaires.
...
* **Scan continu :** Des outils comme Syft, Trivy et des scanners émergents spécifiques à l'IA doivent s'exécuter automatiquement à chaque commit. Si un data scientist met à jour le projet pour utiliser une version plus récente d'un modèle Hugging Face, le pipeline doit automatiquement détecter le changement et régénérer le fichier CycloneDX/SPDX.
