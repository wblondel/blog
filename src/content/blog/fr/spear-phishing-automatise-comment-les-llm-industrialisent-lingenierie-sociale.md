...
Le cycle de vie traditionnel du spear phishing nécessitait qu'un opérateur humain collecte manuellement des informations en source ouverte (OSINT) sur une cible. Aujourd'hui, ce processus est automatisé via des scripts Python enchaînant des outils de scraping avec des API de LLM.
...
Alors que les modèles publics comme GPT-4 ou Claude disposent de garde-fous (RLHF) empêchant la génération de contenu malveillant, les cybercriminels utilisent des modèles spécialisés « non censurés ».
