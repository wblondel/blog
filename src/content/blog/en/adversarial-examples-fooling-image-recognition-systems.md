...
This technology poses a severe threat to biometric security. Researchers have developed 3D-printed adversarial glasses. When an attacker wears these brightly colored frames, they do not just hide their identity—they actively impersonate someone else. 
...
* **Gradient Masking:** Developers attempt to hide the gradient information of the model so attackers cannot calculate the necessary perturbations. However, attackers often bypass this by training a "Substitute Model" (similar to the Model Extraction we covered in Week 11), calculating the noise on the substitute, and applying it to the victim model.
