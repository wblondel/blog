...
## Introduction: The Multi-Million Dollar Heist
...
## 1. The "Teacher-Student" Attack Mechanism

The attack relies on **Knowledge Distillation**. The attacker treats the victim model as a "Black Box" oracle.

1. **Query Generation:** The attacker sends thousands of carefully crafted inputs to the API.
2. **Label Collection:** They record the exact outputs (probability scores or text) returned by the victim.
3. **Training:** They use this dataset `(Input, Output)` to train their own smaller, local model.
...
## 4. Defense: Watermarking and Pattern Detection
